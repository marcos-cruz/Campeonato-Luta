...
namespace Bigai.Torneio.Luta.Api.UnitTests.Models
{
    ...
    {
        [Fact]
        public void Disputar_RetornaCampeaoViceTerceiro()
        {
            // Arrange
            var resultado = 3;
            var campeao = new Lutador() { Id = 2, Nome = "Apollo Creed", Idade = 43, ArtesMarciais = new string[] { "Boxe" }, Lutas = 47, Derrotas = 1, Vitorias = 46 };
            var vice = new Lutador() { Id = 1, Nome = "Adonis Creed", Idade = 32, ArtesMarciais = new string[] { "Boxe" }, Lutas = 26, Derrotas = 1, Vitorias = 25 };
            var terceiro = new Lutador() { Id = 16, Nome = "Eder Jofre", Idade = 82, ArtesMarciais = new string[] { "Boxe" }, Lutas = 81, Derrotas = 2, Vitorias = 77 };
            ...
            // Act
            var vencedores =  lutadores.Disputar();

            // Assert
            Assert.Equal(vencedores.Count, resultado);
            Assert.Equal(campeao.Nome, vencedores[0].Nome);
            Assert.Equal(vice.Nome, vencedores[1].Nome);
            Assert.Equal(terceiro.Nome, vencedores[2].Nome);
        }
    }
}